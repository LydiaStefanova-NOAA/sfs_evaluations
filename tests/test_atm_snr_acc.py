"""
Atmospheric ACC Skill & SNR Potential Predictability Diagnostic Driver
...
def run_atm_acc_snr_diagnostic(
    ...
    end_year: int = 2021,                 # Subsetting end year (e.g. 2000 or 2020)
    detrend: bool = True,                 # Linearly detrend anomalies before computing ACC
    output_png: str = None,
):
    """
    Execute atmospheric ACC skill and SNR predictability diagnostic for a specific target season.
    """
    ...
    logger.info(f"=== Starting {season_str} Atmospheric ACC + SNR Diagnostic ({var_name}) ===")
    ...
    logger.info(f"Loading SFS atmosphere store for Init {init_month:02d}...")
    ds_sfs_raw = get_sfs_data(init_month=init_month, domain="atm", requested_vars=[var_name])
    ...
    sfs_clim = get_or_compute_sfs_climatology(ds_sfs, domain="atm", init_month=init_month)
    ...
    ds_era5_base = ds_era5_raw.sel(time=slice(str(start_year), str(end_year)))
    ...
    era5_season_da = era5_season[var_name].groupby("time.year").mean(dim="time", skipna=True)
    era5_clim_da = era5_season_da.mean(dim="year", skipna=True)
    ...
    if np.issubdtype(era5_season_da.year.dtype, np.datetime64):
        era5_season_da["year"] = era5_season_da.year.dt.year

    # Intersect and filter evaluation window to requested [start_year, end_year]
    common_years = np.intersect1d(sfs_season_da.year.values, era5_season_da.year.values)
    ...
    era5_season_da = era5_season_da.sel(year=eval_years)
    ...
    acc_da = compute_acc(
        ...
        obs_da=era5_season_da,
        sfs_clim=sfs_clim_da,
        obs_clim=era5_clim_da,
        detrend=detrend,
    )
